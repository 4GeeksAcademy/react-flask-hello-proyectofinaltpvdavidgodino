...
function PricePad({ value = "0", onClose, onAccept }) {
  ...
  function press(n) {
    ...
    if (n === ".") { if (!txt.includes(".")) setTxt(txt + "."); return; }
    setTxt((t) => (t === "0" ? String(n) : t + String(n)));
  }
  function accept() {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:11}}>
      <div style={{ background:"#fff", padding:16, width:320, borderRadius:8 }}>
        ...
        <div style={{border:"1px solid #ccc",borderRadius:6,padding:12,fontSize:24,textAlign:"right",marginBottom:12}}>
          {txt}
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:8, marginBottom:12 }}>
          {keys.map((k)=>(<button key={k} onClick={()=>press(k)} style={{padding:12,borderRadius:8}}>{k}</button>))}
        </div>
...
export default function AdminCatalog() {
  ...
  async function loadCategorias() {
    ...
    if (!selectedCat && data.length > 0) {
      // No seleccionamos subcat automáticamente (evita “Pizzas” al cambiar)
  ...
  async function loadSubcategorias(categoria_id) {
    ...
    // NO autoseleccionar; esperamos a que el usuario elija
    setSelectedSub(null);
    setProductos([]);
  }
  ...
  // Cuando cambia categoría: limpiamos selección y cargamos sus subcats
  useEffect(() => {
    if (!selectedCat) return;
    setSelectedSub(null);
    setProductos([]);
    loadSubcategorias(selectedCat.id);
  }, [selectedCat?.id]);

  // Cuando cambia subcategoría: cargamos sus productos
  useEffect(() => {
    if (!selectedSub) { setProductos([]); return; }
    loadProductos(selectedSub.id);
  }, [selectedSub?.id]);
  ...
  function onStartEditCat(c) { setEditCatId(c.id); setEditCatName(c.nombre); }
  ...
  async function onDeleteCat(c) {
    ...
    try {
      ...
      await loadCategorias();
    } catch (e) { alert(e?.error || "No se pudo eliminar la categoría"); }
  }
  ...
  async function onCreateSub() {
    ...
    } catch (e) { alert(e?.error || "No se pudo crear la subcategoría"); }
  }
  function onStartEditSub(s) { setEditSubId(s.id); setEditSubName(s.nombre); }
  async function onSaveEditSub() {
    try {
      ...
      await loadSubcategorias(selectedCat?.id);
    } catch (e) { alert(e?.error || "No se pudo editar la subcategoría"); }
  }
  async function onDeleteSub(s) {
    ...
    try {
      ...
      await loadSubcategorias(selectedCat?.id);
    } catch (e) { alert(e?.error || "No se pudo eliminar la subcategoría"); }
  }
  ...
  async function onCreateProd() {
    ...
    if (newProdPrice == null) return setShowPad(true);
    try {
  ...
  async function onSaveEditProd() {
    try {
      ...
      setEditingProd(null);
      await loadProductos(selectedSub?.id);
    } catch (e) { alert(e?.error || "No se pudo editar el producto"); }
  }
  async function onDeleteProd(p) {
    if (!confirm(`Eliminar producto "${p.nombre}"?`)) return;
    try { await apiDelete(`/admin/catalogo/productos/${p.id}`); await loadProductos(selectedSub?.id); }
    catch (e) { alert(e?.error || "No se pudo eliminar el producto"); }
  }
    ...
    <div style={{ padding: 24, fontFamily: "sans-serif" }}>
      ...
      <div style={{ display:"grid", gap:16, gridTemplateColumns:"1fr 1fr 2fr" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6, maxHeight:320, overflow:"auto" }}>
            {categorias.map(c=>(
              <div key={c.id} style={{
                ...
                <div onClick={()=>setSelectedCat(c)} style={{ cursor:"pointer", flex:1 }}>{c.nombre}</div>
                {editCatId===c.id ? (
                  <>
        ...
        <section style={{ border:"1px solid #ddd", borderRadius:8, padding:12 }}>
          ...
          {selectedCat ? (
            <>
              <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                <input placeholder={`Nueva subcategoría en "${selectedCat.nombre}"`} value={newSubName} onChange={(e)=>setNewSubName(e.target.value)} />
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6, maxHeight:320, overflow:"auto" }}>
                {subcategorias.map(s=>(
                  <div key={s.id} style={{
                    ...
                    <div onClick={()=>setSelectedSub(s)} style={{ cursor:"pointer", flex:1 }}>{s.nombre}</div>
                    {editSubId===s.id ? (
                      <>
        ...
        <section style={{ border:"1px solid #ddd", borderRadius:8, padding:12 }}>
          <h3 style={{ marginTop:0 }}>
            Productos{selectedSub ? ` · ${selectedSub.nombre}` : ""}
          </h3>
          {selectedSub ? (
            <>
              <div style={{ display:"grid", gridTemplateColumns:"1.2fr .8fr auto", gap:8, marginBottom:8 }}>
                <input placeholder={`Nuevo producto en "${selectedSub.nombre}"`} value={newProdName} onChange={(e)=>setNewProdName(e.target.value)} />
                <button onClick={()=>setShowPad(true)}>
                  {newProdPrice == null ? "Precio" : `${Number(newProdPrice).toFixed(2)} €`}
                </button>
              ...
              <div style={{ display:"grid", gap:8 }}>
                {productos.map(p=>(
                  <div key={p.id} style={{ display:"grid", gridTemplateColumns:"1fr 110px auto auto", gap:8, alignItems:"center", border:"1px solid #eee", borderRadius:6, padding:8 }}>
                    {editingProd?.id===p.id ? (
                      <>
                        <input value={editingProd.nombre} onChange={(e)=>setEditingProd({ ...editingProd, nombre:e.target.value })} />
      ...
      {showPad && (
        <PricePad
          ...
          onAccept={(price)=>{ setNewProdPrice(price); setShowPad(false); }}
        />
      )}
